...
export class NgxTouchKeyboardComponent {
  ...
  handleButtonClicked(button: string, e?: Event): void {
    ...
    var commonParams: [number, number, boolean] = [
  ...
  private _setCaretPosition(
    ...
    endPosition = position
     ...
     //begin customized by Datex
    if(this._activeInputElement?.type == 'number' && (!position || position===0) ){
      var val = this._activeInputElement.value.toString();
      position = val.length;
      endPosition = val.length;
    } //end customized by Datex
  ...
  private _addStringAt(
    ...
    //begin customized by Datex
    let isNumber = this._activeInputElement?.type == 'number';
  ...
  private _focusActiveInput(): void {
    this._activeInputElement?.focus();
      //begin customized by Datex
    if(this._activeInputElement?.type=='number'){
      return;
    } //end customized by Datex
